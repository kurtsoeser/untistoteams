(function () {
    ...
    function resolveMsalConfig() {
        ...
        if (!id) {
            throw new Error(
                'Keine clientId: ms365-config.js fehlt/leer oder blockiert. Seite mit Strg+F5 neu laden; im Netzwerk-Tab prüfen, ob ms365-config.js mit 200 lädt. Alternativ meta ms365-graph-client-id setzen (Entra-Anwendungs-ID).'
            );
        }
        return {
            clientId: id,
    ...
    async function runJgOnline() {
        ...
        if (typeof snapshot !== 'function') {
            appendLog('Interner Fehler: Jahrgangs-Daten nicht verfügbar.', 'err');
            return;
        }
        const pack = snapshot();
        if (!pack || !pack.rows || !pack.rows.length) {
            appendLog('Keine Klassen – bitte Liste, Besitzer und Einstellungen abschließen.', 'err');
            return;
        }
        ...
        if (missing.length) {
            appendLog('Bitte für alle Klassen einen Besitzer (UPN) eintragen.', 'err');
            return;
        }
